/*
This file is a C-language reference implementation of GPT-2.
It is based on a CUDA implementation and is provided here for reference purposes.
It is not intended to be compiled and run on a CPU without significant modifications.
For a CPU-based implementation, please refer to the Python scripts in this repository.
*/
...
// These are CUDA specific headers. A CPU version would need to replace these.
...
void gpt2_forward(GPT2 *model, int* inp, int* targets, int B, int T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        // matmul_forward(c_proj_out, att_out, model->c_proj_w + l*C*C, model->c_proj_b + l*C, B, T, C, C); // TODO: where is att_out defined?
        // residual 1
