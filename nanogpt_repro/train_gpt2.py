...
# from hellaswag import render_example, iterate_examples # disabling for now
# -----------------------------------------------------------------------------
...
class DataLoaderLite:
    def __init__(self, B, T, process_rank, num_processes, split, data_dir="edu_fineweb10B", master_process=False):
        self.B = B
        self.T = T
        ...
        if len(shards) > 0:
            ...
            print(f"loaded {len(self.tokens)} tokens from tinyshakespeare")
            self.shards = None


        if self.shards and master_process:
    ...
    def next_batch(self):
        B, T = self.B, self.T
        while True:
            # attempt to fetch a buffer of tokens
            buf = self.tokens[self.current_position : self.current_position+B*T+1]
            if len(buf) == B*T+1:
                break # buffer is good, exit the loop
            # buffer is too short, we are at the end of the current token stream
            if self.shards:
                self.current_shard = (self.current_shard + 1) % len(self.shards)
                self.tokens = load_tokens(self.shards[self.current_shard])
                self.current_position = self.B * self.T * self.process_rank
            else: # tinyshakespeare, just loop
                self.current_position = self.B * self.T * self.process_rank
...
def main():
    ...
    B = 4 # micro batch size, was 8
    T = 256 # sequence length, was 512
    ...
    if use_compile and not ddp: # torch.compile seems not to work with DDP
    ...
    warmup_steps = 10
    max_steps = 50 
    if os.path.exists("edu_fineweb10B"):
        warmup_steps = 715
        max_steps = 19073
